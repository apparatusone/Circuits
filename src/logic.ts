...
export class Simulate {
    ...
    addConnection(source:Type.ComponentType, sourceNode:string, target:Type.ComponentType, targetNode:string) {
        ...
        if (source === target) {
            showAlert('Drawing to the same component is not allowed', 'red')
...
class NandGate extends Generic {
    ...
    logic() {
        const result = this.inputNodeList.reduce((accumulator, input) => accumulator & this[input], 1);
        const newState = result ? 0 : 1;
        this.state = newState;
        return this.state
    }
}
...
class Multiplexor extends Generic {
    constructor(x: number, y: number, r = 0, inCount = 3, outCount = 1) {
        ...
        this.nodes = {
            input_a: {x: -0.5, y: 0.25},
            input_b: {x: -0.5, y: -0.25},
            input_sel: {x: 0, y: -0.5},
            output_a: {x: 0.5, y: 0},
        }
    }
